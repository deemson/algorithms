	...
	for step >= 1 {
		...
		for outerLoopIndex := step; outerLoopIndex < len(slice); outerLoopIndex++ {
			...
			for innerLoopIndex := outerLoopIndex; innerLoopIndex > step-1; innerLoopIndex -= step {
				...
				if !less(slice[innerLoopIndex], slice[innerLoopIndex-step]) {
					break
				}
				swap(slice, innerLoopIndex, innerLoopIndex-step)
			}
		}
		step /= 3
	}
