...
func QuickSort[T any](indexed alg02indexed.Indexed[T], less LessFunc[T]) {
	shuffle(indexed)
	quickSort(indexed, less, 0, indexed.Size()-1)
}

func shuffle[T any](indexed alg02indexed.Indexed[T]) {
	for index := 0; index < indexed.Size(); index++ {
		shuffledIndex := rand.Intn(index + 1)
		alg02indexed.Swap(indexed, index, shuffledIndex)
	}
}

func quickSort[T any](indexed alg02indexed.Indexed[T], less LessFunc[T], left, right int) {
	if left >= right {
		return
	}
...
func partition[T any](indexed alg02indexed.Indexed[T], less LessFunc[T], left, right int) int {
	...
	for {
		...
		alg02indexed.Swap(indexed, leftMarker, rightMarker)
	}
	// put partitioned element in place
	alg02indexed.Swap(indexed, left, rightMarker)
