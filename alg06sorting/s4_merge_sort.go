...
func MergeSort[T any](indexed alg02indexed.Indexed[T], less LessFunc[T]) {
	aux := alg02indexed.SliceAdapter[T]{
...
func BottomUpMergeSort[T any](indexed alg02indexed.Indexed[T], less LessFunc[T]) {
	aux := alg02indexed.SliceAdapter[T]{
		Slice: make([]T, indexed.Size()),
	}
	...
	for partitionSize < indexed.Size() {
		for lo := 0; lo < indexed.Size()-partitionSize; lo += 2 * partitionSize {
			mid := lo + partitionSize - 1
			hi := min(lo+2*partitionSize-1, indexed.Size()-1)
...
func mergeSort[T any](indexed, aux alg02indexed.Indexed[T], less LessFunc[T], lo, hi int) {
...
func merge[T any](indexed, aux alg02indexed.Indexed[T], less LessFunc[T], lo, mid, hi int) {
