...
object S3ShellSort : Sort {
    override fun <T> sort(sortable: Sortable<T>, comparator: Comparator<T>) {
        ...
        while (step >= 1) {
            ...
            for (outerIndex in step until sortable.size) {
                ...
                for (innerIndex in outerIndex downTo step step step) {
                    ...
                    if (comparator.compare(sortable[innerIndex], sortable[innerIndex - step]) >= 0) {
                        break
                    }
                    sortable.swap(innerIndex, innerIndex - step)
                }
            }
            step /= 3
        }
